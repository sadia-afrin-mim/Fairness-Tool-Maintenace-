...
# Load the CSV file
csv_file_path = 'ESEM - Dataset.csv'  # Replace with your CSV file path
...
def get_repo_info(repo_name):
    try:
        ...
        # Print a message after processing the repository
...
# Loop over each repository in the CSV and get information
repo_data = []
for repo_name in df['Repository']:
    info = get_repo_info(repo_name)
    if info:
        repo_data.append(info)

# Convert results to a DataFrame for easier analysis and export
results_df = pd.DataFrame(repo_data)

# Save to a new CSV file
output_csv = 'repository_info_output.csv'
results_df.to_csv(output_csv, index=False)

print(f"Repository information saved to {output_csv}")
